...
import "../dependencies/contracts/token/ERC20/IERC20.sol";
...
contract PancakePair is IPancakePair  {
    ...
    function _mintFee(uint112 _reserve0, uint112 _reserve1) private returns (bool feeOn) {
        ...
        if (feeOn) {
            if (_kLast != 0) {
                uint rootK = SelfMath.sqrt(uint(_reserve0).mul(_reserve1));
                uint rootKLast = SelfMath.sqrt(_kLast);
                if (rootK > rootKLast) {
    ...
    function mint(address to) external lock returns (uint liquidity) {
        ...
        if (_totalSupply == 0) {
            liquidity = SelfMath.sqrt(amount0.mul(amount1)).sub(MINIMUM_LIQUIDITY);
            _mint(address(0), MINIMUM_LIQUIDITY); // permanently lock the first MINIMUM_LIQUIDITY tokens
        } else {
            liquidity = SelfMath.min(amount0.mul(_totalSupply) / _reserve0, amount1.mul(_totalSupply) / _reserve1);
        }
...
library PancakeLibrary {
    ...
    function pairFor(address factory, address tokenA, address tokenB) internal view returns (address pair) {
        ...
        /*uint ret = uint(keccak256(abi.encodePacked(
                hex'ff',
                factory,
                keccak256(abi.encodePacked(token0, token1)),
                hex'00fb7f630766e6a796048ea87d01acd3068e8ff67d078148a3fa3f4a84f69bd5' // init code hash
            )));
        pair = address(uint160(ret));*/
